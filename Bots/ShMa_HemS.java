...
public class ShMa_HemS extends HolsDerGeierSpieler {
	...
	public int waehleKarte(int naechsteKarte) {
		...
		if (naechsteKarte >= 7 || naechsteKarte == -5) {
			...
			for (int i = 15; i >= 11; i--) {
				//System.println("Max großer pool wir" + max);
				//System.println("MaxGegner großer pool wir" + maxGegner);
				if (maxGegner == max) {
		...
		if (naechsteKarte >= 4 && naechsteKarte < 7 || naechsteKarte == -4
			...
			for (int i = 10; i >= 6; i--) {
				//System.println("Max mittlerer pool wir" + max);
				//System.println("MaxGegner mittlerer pool wir" + maxGegner);
				if (maxGegner == max) {
					return austauschen(max);
				}
				if (maxGegner > max) {
		...
		if (naechsteKarte >= 1 && naechsteKarte < 4 || naechsteKarte == -1
			...
			for (int i = 5; i >= 1; i--) {
				//System.println("Max kleinerer pool wir" + max);
				//System.println("MaxGegner kleinerer pool wir" + maxGegner);
				if (maxGegner == max) {
					return austauschen(max);
				}
				if (maxGegner > max) {
	...
	public int gibKarte(int naechsteKarte) {
		int ret = -99;
		int letzteKarteGegner = getHdg().letzterZug(getNummer() == 0 ? 1 : 0);
