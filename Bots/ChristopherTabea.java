...
public class ChristopherTabea extends HolsDerGeierSpieler{
	...
	public int gibKarte (int naechsteKarte)
	{
		...
		if (runde>0){
			if(vomGegnerNochNichtGelegt.contains(getHdg().letzterZug(getNummer() == 0 ? 1 : 0)))
			{
				vomGegnerNochNichtGelegt.remove(vomGegnerNochNichtGelegt.indexOf(getHdg().letzterZug(getNummer() == 0 ? 1 : 0)));
				schonGespieltGegner.add(getHdg().letzterZug(getNummer() == 0 ? 1 : 0));
			}
		}
		if (runde>0)
			punkteZaehlen();
